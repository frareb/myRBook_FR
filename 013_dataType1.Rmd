...
Attention, il y a un piège à l'utilisation de la fonction `is.integer`. Elle ne vous dit pas si le nombre est un entier mais si il est du type `integer`. En effet, on peut très bien stocker un nombre entier dans une variable de type `double`.
...
On ne peut pas quitter cette section sans mentionner les fonctions `ceiling`, `floor`, `trunc` ou `round` qui retournent la partie entière d'un nombre mais le laisse au type `double`. Pour en savoir plus, faites `?round`.
