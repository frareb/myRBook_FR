...
Le premier type de graphique que nous allons voir est le nuage de points. Dans un nuage de points, chaque point est représenté par sa valeur en x et en y. La fonction permettant de faire un nuage de points est `plot()`. Il est recommandé de spécifier les arguments `x` et `y` même si ceux-ci ne sont pas nécessaires. Cela permet de gagner en lisibilité et d’éviter les erreurs (Figure \@ref(fig:021-01-graphiques-1)).
...
Comme pour tous les types de graphiques, nous pouvons ajouter une légende sur l'axe des x et des y (Figure \@ref(fig:021-01-graphiques-2)). Par défaut les axes prennent le nom des objets utilisés pour le graphique, dans la figure \@ref(fig:021-01-graphiques-1), `myX` et `myY`.
...
Nous pouvons aussi définir les limites des axes en X et en Y. Ici nous représentons l’axe des X entre -3 et 3, et l’axe des Y entre 7 et 13 (Figure \@ref(fig:021-01-graphiques-3)).
...
La couleur des points peut se définir avec l'argument `col` (Figure \@ref(fig:021-01-graphiques-6)). Nous reviendrons sur les couleurs dans un prochain chapitre.

```{r 021-01-graphiques-6, out.width = '70%', fig.cap = "Couleur des points (col) ; voir cahier en couleurs"}
...
```{r 021-01-graphiques-7, out.width = '70%', fig.cap="Couleur et taille des points (col) ; voir cahier en couleurs"}
myX <- rnorm(100)
myY <- rnorm(100)
...
Une dernière option très utile est l'argument `panel.first` qui permet de réaliser une opération graphique sur une couche située en dessous de notre graphique. Voici un exemple illustratif avec une grille réalisée avec et sans `panel.first` (Figure \@ref(fig:021-01-graphiques-9)). La grille se fait grâce à la fonction `grid()`. Pour mettre les graphiques côte à côte nous allons utiliser `mfrow`.
...
Il est souvent utile de faire figurer des lignes verticales ou horizontales pour représenter des valeurs particulières (Figure \@ref(fig:021-01-graphiques-11)). Ces lignes peuvent être ajoutées avec la fonction `abline()`. 

```{r 021-01-graphiques-11, out.width='70%', fig.cap="Lignes verticales et horizontales avec abline() ; voir cahier en couleurs"}
myX <- rnorm(100)
myY <- rnorm(100)
par(mar = c(2, 2, 0, 0)) 
plot(x = myX, y = myY, xlab = "", ylab = "",
...
Pour faire un histogramme nous utilisons la fonction `hist()` (Figure \@ref(fig:021-01-graphiques-12)). C'est une fonction graphique utile pour visualiser rapidement la distribution d'un jeu de données (nous reviendrons sur la gestion des couleurs dans un autre chapitre et notamment sur l'utilisation de `heat.colors()`). 

```{r 021-01-graphiques-12, fig.cap="Histogramme avec hist() ; voir cahier en couleurs"}
op <- par(no.readonly = TRUE)
...
Quand l'objet envoyé à cette fonction est un `vector` alors la fonction `barplot()` renvoie un graphique en barres simples. Quand c'est une `matrix` alors les barres sont superposées (Figure \@ref(fig:021-01-graphiques-14)).
...
```{r 021-01-graphiques-15, fig.cap="Graphique composé ; voir cahier en couleurs"}
n <- 50
myX <- rnorm(n)
myY <- rnorm(n)
...
Nous pouvons ensuite intégrer ce script dans une fonction pour par exemple étudier l'effet de la variable `n` (Figure \@ref(fig:021-01-graphiques-16)).

```{r 021-01-graphiques-16, fig.cap="Graphique sous forme de fonction ; voir cahier en couleurs"}
...
```{r 021-01-graphiques-17, out.width = '70%', fig.cap="Barplot avec valeurs négatives ; voir cahier en couleurs"}
par(mar = c(3, 1, 1, 1)) # réduction des marges
barplot(rnorm(20), horiz = TRUE, col = rainbow(20))
```

Le `barplot` peut aussi être utilisé pour faire une pyramide des âges (il existe des fonctions pour réaliser des pyramides des âges, ici l'objectif est pédagogique ; Figure \@ref(fig:021-01-graphiques-18)).

```{r 021-01-graphiques-18, out.width = '70%', fig.cap="Pyramide des âges ; voir cahier en couleurs"}
gender <- data.frame(
...
Si une variable est de type `factor` ou `character`, la fonction `boxplot()` permet facilement de représenter chaque catégorie. Cela fonctionne aussi avec les variables numériques mais il faut veiller à ne pas avoir trop de valeurs différentes pour que le graphique reste lisible (Figure \@ref(fig:021-01-graphiques-20))).

```{r 021-01-graphiques-20, out.width = '70%', fig.cap="Boxplot par catégorie ; voir cahier en couleurs"}
df$cat <- sample(
  c("w", "x", "y", "z"), 
  size = 1000, 
  replace = TRUE)
