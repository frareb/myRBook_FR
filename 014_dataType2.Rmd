...
Le conteneur `data.frame` peut être assimilé à un *tableau*. Il s'agit en fait d'un cas particulier de `list` où tous les élément de la `list` ont la même longueur. 
...
Il est possible de créer une `matrix` à partir d'un `data.frame` avec la fonction `as.data.frame` sous réserve que le `data.frame` ne contienne que des élément de même type (par exemple que des éléments de type `numeric`).
...
Le conteneur de type `array` est une généralisation du conteneur de type `matrix`. Là où le type `matrix` a deux dimensions (les lignes et les colonnes), le type `array` a un nombre indéfini de dimensions.
