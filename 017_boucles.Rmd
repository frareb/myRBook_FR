...
Lorsque il y a plusieurs lignes de codes à exécuter en fonction du test logique, ou simplement pour rendre le code plus facile à lire, nous allons utiliser plusieurs lignes avec les acolades `{}` et en utilisant l'indentation. Nous pouvons retenir comme règle de présentation qu'une ligne de code de plus de 80 caractères (voire 60) doit être découpée en plusieurs lignes. Les éditeurs de texte les plus communs permettent la visualisation d'une ligne verticale à 80 ou 60 caractères pour symboliser cette barrière.
...
Comme la boucle `while()`, la boucle `repeat()` reste peu utilisée avec R, sauf dans certaines disciplines en lien avec des données courantes par exemple.
...
Et si nous voulons le résultat sous forme d'un `data.frame`, le code devient beaucoup plus long et fastidieux (le code suivant correspond à une solution parmi d'autres). Là encore nous voyons toute la puissance des boucles de la famille `apply`.
