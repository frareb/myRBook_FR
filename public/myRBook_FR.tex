...
éléments ne peuvent se déduire que grâce aux parenthèses. C'est pourquoi
...
nous souhaitons récupérer plusieurs éléments d'une liste. Par exemple
pour sélectionner les abondances d'insectes des deux premiers sites de
la première espèce, nous utiliserons
...
Une liste peut être modifiée de la même façon que pour le conteneur
\texttt{vector}, c'est à dire en se réferrant avec des crochets à
l'élément que nous souhaitons modifier.
...
avec la fonction \texttt{unlist()} si le format des éléments de la liste
le permet (un \texttt{vector} ne peut contenir que des élément du même
type).
...
\section{Plus d'information sur les
objets}\label{plus-dinformation-sur-les-objets}

XXX rm ls \ldots{}

\chapter{Les fonctions}\label{fonctions}

\section{Quest-ce qu'une fonction}\label{quest-ce-quune-fonction}
...
\section{La boucle if}\label{la-boucle-if}

XXX

\section{La boucle switch}\label{la-boucle-switch}
...
\section{repeat, next, break, stop}\label{repeat-next-break-stop}

XXX

\section{Les boucles de la famille
apply}\label{les-boucles-de-la-famille-apply}
