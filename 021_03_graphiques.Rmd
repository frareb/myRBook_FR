...
```{r 021-03-graphiques-2, out.width = '70%'}
pkgCheck <- function(x){ 
  if (!require(x, character.only = TRUE)){
    install.packages(x, dependencies = TRUE)
    if(!require(x, character.only = TRUE)) {
...
```{r 021-03-graphiques-3, fig.width=11, fig.height=6}
pkgCheck <- function(x){ 
  if (!require(x, character.only = TRUE)){
    install.packages(x, dependencies = TRUE)
    if(!require(x, character.only = TRUE)) {
...
```{r 021-03-graphiques-4, fig.width=11, fig.height=6}
# ggplot2
p <- ggplot(data = iris, aes(
  x = Sepal.Length, 
  y = Sepal.Width, 
...
```{r 021-03-graphiques-5, fig.width=11, fig.height=6}
# linear regressions
...
```{r 021-03-graphiques-5b, fig.width=11, fig.height=6}
par(mar = c(4, 4, 2, 8))
plot(x = iris$Sepal.Length, y = iris$Sepal.Width, 
